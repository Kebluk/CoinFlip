    ...
    async initialize(): Promise<void> {
        return new Promise((resolve, reject) => {
            this.db.serialize(() => {
                ...
                this.db.get(`SELECT balance FROM wallet`, (err, row: any) => {
                    if (err) return reject(err);
                    if (!row) {
                        console.log(
                            chalk.magenta(
                                "It looks like you don't have any balance, but don't worry, our LEGAL bank has decided to give you 1000 completely LEGAL FlipCoins to get you started. Enjoy!"
                            )
                        );
...
function randomIntIncl(min: number, max: number): number {
...
enum Faq {
    REAL = "Is the gambling real?",
    TRUST = "Can I trust you?",
    WIN = "What happens if I win?",
    MONEY = "How can you afford this?",
    QUIT = "Quit"
}

function coinSideToString(side: CoinSide): string {
...
async function getExtraBalance(): Promise<void> {
...
async function main() {
    ...
    program
        ...
        .action(async (options: { side: string | undefined; amount: string | undefined }) => {
                ...
                const answers = await inquirer.prompt([
                    {
                        ...
                        validate: (input) =>
                            (input !== undefined && input > 0 && input <= balance) ||
            ...
            let side: CoinSide | undefined;
            if (sideOption == "heads" || sideOption == "h") side = CoinSide.HEADS;
            else if (sideOption == "tails" || sideOption == "t") side = CoinSide.TAILS;
            else side = undefined;
            ...
            async function faqSelectLoop() {
                ...
                switch (faq) {
                    case Faq.REAL:
                        console.log(
                            chalk.cyan(
                                "A: No, not for you. Your actions are just privately transmitted through the dark side of the internet (you'd be right, if you guessed Tor, that's why everything is so slow) and some random people are forced to follow your actions, if they do not, we all know, what will happen to them."
                            )
                        );
                        break;
                    case Faq.TRUST:
                        console.log(
                            chalk.cyan(
                                "A: If you trust tech companies, that are potentially selling your data, I don't see a problem to trust us."
                            )
                        );
                        break;
                    case Faq.WIN:
                        console.log(
                            chalk.cyan("A: If you win, you get money, if you lose, you get nothing. I don't see any problem in here.")
                        );
                        break;
                    case Faq.MONEY:
                        console.log(
                            chalk.cyan(
                                "A: Haven't you ever heard of not asking questions that you don't want answered? No? Okay. So, let's just say we have our ways. It involves a lot of magic and maybe, just maybe, some questionable ethics."
                            )
                        );
                        break;
                    case Faq.QUIT:
